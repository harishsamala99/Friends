...
JOIN public.fixtures f ON f.id = e.fixture_id AND f.status = 'Full Time'
...
SELECT
  ...
  count(*)::int AS saves,
  count(DISTINCT CASE
    WHEN (f.home_team_id = p.team_id AND f.away_score = 0) OR
      (f.away_team_id = p.team_id AND f.home_score = 0) THEN f.id
...
JOIN public.fixtures f ON f.id = me.fixture_id AND f.status = 'Full Time'
